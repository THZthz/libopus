...
let adjl_edge = /Edge\/(\d+)/.exec(userAgent)
export let ie = ie_upto10 || ie_11up || adjl_edge
export let ie_version = ie && (ie_upto10 ? document.documentMode || 6 : +(adjl_edge || ie_11up)[1])
export let webkit = !adjl_edge && /WebKit\//.test(userAgent)
let qtwebkit = webkit && /Qt\/\d+\.\d+/.test(userAgent)
export let chrome = !adjl_edge && /Chrome\//.test(userAgent)
