...
#define ALLOC__(type, dst, ele_count)                                    \
	...
	data->hash_size_ = r_ceil(r_sqrt(n));
...
static INLINE void get_bounding_box_(const real *coords, uint32_t n, uint32_t *ids,
                                     real *min_x, real *min_y, real *max_x, real *max_y, real *cx, real *cy)
{
	uint32_t i;
...
static INLINE real square_dist(real ax, real ay, real bx, real by)
{
	return (ax - bx) * (ax - bx) + (ay - by) * (ay - by);
}

static INLINE uint32_t get_closet_point0_(const real *coords, uint32_t n, real x, real y)
{
	uint32_t i, min_i = -1;

	real min_dist = REAL_MAX;
	for (i = 0; i < n; i++) {
		real d = square_dist(x, y, coords[2 * i], coords[2 * i + 1]);
		if (d < min_dist) {
			min_i    = i;
			min_dist = d;
		}
...
static INLINE uint32_t get_closet_point1_(const real *coords, uint32_t n, real x, real y, uint32_t f)
{
	uint32_t i, min_i = -1;
...
static INLINE uint32_t get_smallest_point_(const real *coords, uint32_t n,
                                           uint32_t a, uint32_t b,
                                           real ax, real ay, real bx, real by,
                                           real *min_r)
{
	uint32_t i, min_i;
	real     cx, cy, r_sq, min_r_sq = REAL_MAX;
	for (i = 0; i < n; i++) {
		if (i == a || i == b) continue;
		get_circum_center_of_triangle(ax, ay, bx, by, coords[2 * i], coords[2 * i + 1], &cx, &cy, &r_sq);
		if (r_sq < min_r_sq) {
			min_i    = i;
			min_r_sq = r_sq;
		}
...
static INLINE void swap_id_(uint32_t *ids, uint32_t i, uint32_t j)
{
	uint32_t t = ids[i];
	ids[i]     = ids[j];
	ids[j]     = t;
...
static void quicksort_(uint32_t *ids, real *dists, uint32_t left, uint32_t right)
{
	int64_t  i, j;
	uint32_t temp;
	real     temp_dist;
...
static INLINE real orient2d_(real x1, real y1, real x2, real y2, real x3, real y3)
{
...
static INLINE void sort_counter_clock_wise_(real i0x, real i0y, real *i1x, real *i1y, real *i2x, real *i2y, uint32_t *i1, uint32_t *i2)
{
...
static INLINE uint32_t get_hash_key_(real x, real y, real cx, real cy, uint32_t hash_size)
{
	real dx = x - cx, dy = y - cy;
	real p = dx / r_abs(dx) + r_abs(dy);
	p      = (dy > 0 ? 3 - p : 1 + p) / 4;
	return (uint32_t) r_floor(p * hash_size) % hash_size;
}

static INLINE void add_linK_(delaunay_data_t *data, uint32_t a, uint32_t b)
{
...
static INLINE uint32_t add_triangle_(delaunay_data_t *data,
...
static INLINE uint32_t legalize_(delaunay_data_t *data, uint32_t a)
{
	uint32_t i  = 0;
	uint32_t ar = 0;
...
void delaunay_triangulate(delaunay_data_t *data)
{
	...
	if (min_r == REAL_MAX) {
		...
		for (i = 0; i < data->n >> 1; i++)
			dists[i] = (real) ((coords[2 * i] - coords[0]) || (coords[2 * i + 1] - coords[1]));

		quicksort_(ids, dists, 0, (data->n >> 1) - 1);
		hull = (uint32_t *) malloc(sizeof(uint32_t) * (data->n >> 1));
		j    = 0;
		for (i = 0, d = -REAL_MAX; i < data->n >> 1; i++) {
			uint32_t id = ids[i];
			if (dists[id] > d) {
				hull[j] = id;
				d       = dists[id];
	...
	get_circum_center_of_triangle(i0x, i0y, i1x, i1y, i2x, i2y, &cx, &cy, &r_sq);
	...
	for (k = 0; k < data->n >> 1; k++) {
		...
		if (k > 0 && r_abs(x - xp) <= EPSILON && r_abs(y - yp) <= EPSILON) continue;
		xp = x;
		yp = y;
