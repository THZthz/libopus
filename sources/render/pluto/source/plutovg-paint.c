...
plutovg_paint_t* plutovg_paint_create_rgba(double r, double g, double b, double a)
{
    ...
    paint->u_.color = malloc(sizeof(plutovg_color_t));
    plutovg_color_init_rgba(paint->u_.color, r, g, b, a);
    return paint;
}
...
plutovg_paint_t* plutovg_paint_create_gradient(plutovg_gradient_t* gradient)
{
    ...
    paint->u_.gradient = plutovg_gradient_reference(gradient);
    return paint;
}
...
plutovg_paint_t* plutovg_paint_create_texture(plutovg_texture_t* texture)
{
    ...
    paint->u_.texture = plutovg_texture_reference(texture);
    return paint;
}
...
void plutovg_paint_destroy(plutovg_paint_t* paint)
{
    ...
    if(--paint->ref == 0)
    {
        ...
        if(paint->type == plutovg_paint_type_gradient)
            plutovg_gradient_destroy(paint->u_.gradient);
        if(paint->type == plutovg_paint_type_texture)
            plutovg_texture_destroy(paint->u_.texture);
        free(paint);
    }
}
...
{
    return paint->type == plutovg_paint_type_color ? paint->u_.color : NULL;
}

plutovg_gradient_t* plutovg_paint_get_gradient(const plutovg_paint_t* paint)
{
    return paint->type == plutovg_paint_type_gradient ? paint->u_.gradient : NULL;
}

plutovg_texture_t* plutovg_paint_get_texture(const plutovg_paint_t* paint)
{
    return paint->type == plutovg_paint_type_texture ? paint->u_.texture : NULL;
}
